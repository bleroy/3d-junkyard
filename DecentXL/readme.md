...
Whether you have a vintage computer with a deficient keyboard or just want a better keyboard, this is the project for you.

There are two versions of the keyboard: modern and vintage. Modern uses brand new MX-compatible key caps and switches, while vintage keeps the original vintage Atari caps and adapts them to modern switches. The vintage version can use Kailh Choc v2 low-profile switches, in which case 3D-printed adapters bridge one of the supported variants of XL key caps to the Choc switches' MX footprint, or [Alps-compatible switches](http://matias.ca/switches/) if you're fortunate enough to own one of the Alps XL keyboards, without needing any adapters.
...
The current revision of the DecentXL keyboard is: rev. 1.
...
The DecentXL project is constituted of the following parts:

* A main PCB with surface-mounted anti-ghosting diodes, adjustable blue backlighting and flexible cable connection.
* A daughter board for the backlighted console keys and white surface-mounted power LED. Connected to the main board with a 10-pin flexible cable.
...
![The DecentXL modern layout](./Media/DecentXL-layout-modern.png)

The key cap set is defined by [this SVG file](./Caps/DecentXL-Maroon-Caps.svg).
...
* Alps:
  ...
  No adapters are necessary, just make sure to buy Alps-compatible switches.
...
62 switches corresponding to your cap type (MX for modern, Alps for vintage Alps, Kailh Choc v2 for non-Alps vintage).
...
Vintage keys will need a stabilizer adapter fit into the key caps so they can attach to the stabilizers. Currently available adapters are:

* Square: [Left shift](); [Space]()
* Cross: [Left shift](); [Space]()
* Alps: [Left shift](); [Space]()

### Main and daughter boards

Use [the Gerber](./DecentXL-keyboard/production-pcb/gerber.zip), [BOM](./DecentXL-keyboard/production-pcb/bom.csv) and [position](./DecentXL-keyboard/production-pcb/positions.csv) files to order a PCB with all the SMD parts assembled. The daughter board detaches from the mainboard.
...
After making sure all the switch pins are vertical and none are bent out of place, carefully align the plate and switches with the stabilizers and the PCB. Slowly and carefully complete that assembly, making sure no pins get bent. Once that is done, the plate and PCB should be a few millimeters apart, more or less evenly spaced across the whole surface. You may want to insert some spacers between them and secure them together with tape or clothespins until you've soldered enough of them.
...
Gently insert the PCBs into the brackets. The screw holes should align. If you're using the modern layout, you'll want to shift the main board a few millimeters to the right to perfectly center the keyboard (the modern layout is 0.25U less wide than the original so need to be re-centered).
...
Align the bracket screw holes with the correct columns on the top shell of your Atari XL case and screw the keyboard in place. If using the modern layout, leave some freedom so you can precisely center your keyboard in its window, then secure the screws in place.
